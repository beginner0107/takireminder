...
### 3-1. 리마인더 목록 표시
- [x] 메인 영역에 선택된 리스트 제목 표시 (34px bold, iOS Large Title)
- [x] `GET /api/lists/{id}/reminders` 연동하여 리마인더 목록 렌더링
- [x] `ReminderItem` 컴포넌트: 원형 체크박스 + 제목 + 메모 미리보기 + 마감일 + 우선순위
- [x] 체크박스 색상을 소속 리스트 색상으로 연동

### 3-2. 리마인더 생성
- [x] 목록 하단 `+ 새로운 리마인더` 클릭 → 인라인 입력 필드 활성화
- [x] 제목 입력 후 Enter → `POST /api/lists/{id}/reminders` 호출
- [x] 생성 후 목록에 즉시 반영

### 3-3. 완료 토글
- [x] 체크박스 클릭 → `PATCH /api/reminders/{id}/complete` 호출
- [x] 완료 애니메이션: 체크 표시 + 취소선 + opacity 0.5
- [x] 0.5초 딜레이 후 목록에서 페이드아웃 제거

### 3-4. 디테일 패널 (편집)
- [x] 리마인더 클릭 시 우측 디테일 패널 슬라이드 인
- [x] 제목 편집 필드
- [x] 메모 편집 필드 (textarea)
- [x] 마감일 날짜 피커
- [x] 우선순위 세그먼트 컨트롤 (없음/낮음/보통/높음)
- [x] 소속 리스트 변경 드롭다운
- [x] Apple 스타일 폼 UI: 둥근 모서리 그룹 카드, 라벨-값 행 구조
- [x] 변경 사항 저장 → `PUT /api/reminders/{id}` 호출

### 3-5. 리마인더 삭제
- [x] 리마인더 아이템 호버 시 삭제 아이콘(✕) 표시
- [x] 클릭 시 확인 후 `DELETE /api/reminders/{id}` 호출
- [x] 삭제 후 목록에서 즉시 제거
