...
- [ ] `globals.css`에 Apple 시스템 폰트 및 색상 변수 정의
- [ ] `src/lib/types.ts` — ReminderList, Reminder, Priority 타입 정의
- [ ] `src/lib/api.ts` — fetch 기반 API 클라이언트 함수 작성

### 2-2. 레이아웃 구현
- [ ] `layout.tsx` — 2-column 레이아웃 (사이드바 280px + 메인 영역)
- [ ] 사이드바 컨테이너: 배경색 `#F2F2F7`, 스크롤 가능

### 2-3. 사이드바 — 스마트 리스트 카드
- [ ] `SmartListGrid` 컴포넌트 — 2×2 그리드 배치
- [ ] 각 카드: 둥근 모서리, 아이콘, 카운트(0), 라벨
- [ ] 카드 색상 적용 (오늘=파랑, 예정=빨강, 전체=검정, 완료됨=회색)

### 2-4. 사이드바 — 사용자 리스트
- [ ] `Sidebar` 컴포넌트 — 리스트 목록 렌더링
- [ ] 리스트 아이템: 색상 원형 아이콘(●) + 이름 + 미완료 카운트
- [ ] 리스트 선택 시 배경 하이라이트
- [ ] `+ 목록 추가` 버튼

### 2-5. 리스트 CRUD 연동
- [ ] 리스트 조회 API 연동 (GET /api/lists → 사이드바에 표시)
- [ ] 리스트 생성 다이얼로그 (이름 입력 + 색상 선택 팔레트)
- [ ] 리스트 수정 (이름/색상 변경)
- [ ] 리스트 삭제 (확인 다이얼로그 포함)
